...
// Homework 4 content:
// 1. Openning book - done
// 2. history Heuristic - done
// 3. Zero-window search - done
...
Agent::Agent() : name("Ash"), my_player(player1) {
...
Move Agent::nextMove() {
	...
	for (i = moves.begin(); i != moves.end();){
		if (!isbackward(current_player, *i)) i = moves.erase(i);
		else ++i;
	}
	...
	for (int i = 0; i < moves.size(); i++){
		...
		if (it != tt.end() && it->second.depth >= depth &&
			...
			if (alpha > 0 && alpha < beta){
				e = minNode(board, newmoves, depth, startTime, beta - 1, beta, tt, moves[i], 0);
			}
			else{
				e = minNode(board, newmoves, depth, startTime, alpha, beta, tt, moves[i], 0);
			}
...
bool Agent::isbackward(Players current_player, Move m){
